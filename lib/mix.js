...
var hashidLen = 4;
var hashids = new Hashids(util.getRandomStr(), hashidLen);
...
Mix.prototype.init = function (opts) {
    ...
    this.randomStrLen = opts.randomStrLen || 6;
    this.randdomStrMaxLen = opts.randdomStrMaxLen || 8;
 ...
 * @return {[type]} [description]
 */
Mix.prototype.paramCheck = function (opts) {
    if (opts.list == null || opts.resource == null
        || opts.getFileContent == null || opts.setFileContent == null) {
        util.errorLog('params [list],[resource],[getFileContent],[setFileContent] is required, pleasr check!');
    }
...
Mix.prototype.setMixStr = function (original, type) {
    ...
    if (checkMix(prefixOriginal, type)) {
        ...
        if (!that.isIgnored(original, type)) {
            ...
            var strLen = this.randomStrLen - hashidLen;
            var strMaxLen = this.randdomStrMaxLen - hashidLen;
            mixStr = mixStr ? mixStr
                : (prefixOriginal != null && this.map[type][prefixOriginal] != null)
                    ? this.map[type][prefixOriginal]
                    : util.getRandomStr(strLen, strMaxLen) + hashids.encode(this.mapCounter);
            if (isCustom) {
